...
1. 配置谷歌学术引用爬虫：
    ...
    1. 在github本仓库页面的`Actions`中启用workflows。本action会把谷歌学术引用统计缓存到`assets/data/gs_data.json`和`assets/data/gs_data_shieldsio.json`，并在每天08:00 UTC定时触发；也可以手动运行。
...
1. 修改主页配置文件[_config.yml](../_config.yml):
    ...
    1. 引用统计数据随站点从`assets/data/gs_data.json`读取，不再需要`google_scholar_stats_use_cdn`配置。
    1. 更多的配置信息在注释中有详细描述。
